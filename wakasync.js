 ...
 * ║                                                                              ║
 * ║  Auto-integrates with WakaPAC when available, injecting control() method    ║
 * ║  directly into every Context object.                                         ║
 * ║                                                                              ║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
...
(function() {
    ...
    /**
     * WakaPAC Context Integration
     * WakaSync automatically injects itself into WakaPAC Context objects
     */
    (function() {
        // Only attempt integration in browser environments
        if (typeof window === 'undefined' || typeof document === 'undefined') {
            return;
        }

        /**
         * Creates a control method for a WakaPAC context
         * @param {Object} context - WakaPAC Context instance
         * @returns {Function} The control method
         */
        function createControlMethod(context) {
            // Create component-scoped HTTP client
            const httpClient = wakaSync.create({
                timeout: 30000,
                retries: 1,
                headers: {
                    'X-Component-ID': context.container.getAttribute('data-pac-container') || 'unknown'
                }
            });

            // Store for cleanup
            context._wakaHttpClient = httpClient;

            // Return the control method (enhanced request method)
            return function control(url, options = {}) {
                const enhancedOptions = {
                    groupKey: context.container.getAttribute('data-pac-container'),
                    latestOnly: true,
                    ignoreAbort: true,
                    ...options
                };
                return httpClient.request(url, enhancedOptions);
            };
        }

        /**
         * Enhances WakaPAC Context prototype with HTTP functionality
         */
        function enhanceContextPrototype() {
            // Check if Context is available
            if (typeof window.Context === 'undefined') {
                return false;
            }

            const ContextProto = window.Context.prototype;

            // Store original constructor
            const OriginalContext = window.Context;

            // Create enhanced constructor
            function EnhancedContext(container, abstraction, config) {
                // Call original constructor
                OriginalContext.call(this, container, abstraction, config);

                // Store reference to context for closures
                const context = this;
                ...
                context.abstraction.cancelRequests = function() {
                    ...
                    if (groupKey && context._wakaHttpClient) {
                        context._wakaHttpClient.cancelGroup(groupKey);
                    }
                };
            }

            // Copy prototype and constructor
            EnhancedContext.prototype = ContextProto;
            EnhancedContext.prototype.constructor = EnhancedContext;

            // Replace the global Context constructor
            window.Context = EnhancedContext;

            // Enhance destroy method for cleanup
            const originalDestroy = ContextProto.destroy;
            if (originalDestroy) {
                ContextProto.destroy = function() {
                    // Cancel HTTP requests
                    if (this._wakaHttpClient) {
                        const groupKey = this.container.getAttribute('data-pac-container');
                        if (groupKey) {
                            this._wakaHttpClient.cancelGroup(groupKey);
                        }
                        this._wakaHttpClient = null;
                    }
        ...
        /**
         * Adds global HTTP utilities to WakaPAC
         */
        function enhanceWakaPAC() {
            if (typeof window.wakaPAC === 'function') {
                window.wakaPAC.http = wakaSync;
                window.wakaPAC.cancelAllRequests = () => wakaSync.cancelAll();
                window.wakaPAC.getActiveRequestCount = () => wakaSync.getActiveRequestCount();
            }
        }
        ...
        function initializeIntegration() {
            if (enhanceContextPrototype()) {
                enhanceWakaPAC();
                console.log('WakaSync: Integrated into WakaPAC Context');
                return true;
            }
            return false;
        }
        ...
        /**
         * Check for WakaPAC and integrate
         */
        function checkForWakaPAC() {
            if (typeof window.wakaPAC === 'function' && typeof window.Context !== 'undefined') {
        ...
        if (!checkForWakaPAC()) {
            // Set up delayed check for dynamic loading
            let attempts = 0;
            const maxAttempts = 20;

            const checkInterval = setInterval(() => {
                attempts++;
                if (checkForWakaPAC() || attempts >= maxAttempts) {
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', checkForWakaPAC);
        }

...
(function () {
     ...
     *   cmdId     — command identifier posted in wParam of MSG_COMMAND
    ...
    /**
     * Attempts to translate a PAC keyboard message into a MSG_COMMAND message
     * using the accelerator table registered for the given container (or the
     * global table if no container match is found).
     *
     * Returns true and dispatches MSG_COMMAND when a match is found, which also
     ...
     *   - Each matched entry posts wParam = (HIWORD=1, LOWORD=cmdId), lParam = 0,
     *     which is identical to WM_COMMAND generated by a menu selection.
     *
    ...
    function _translateAccelerator(pacEvent, container) {
        ...
        for (const table of tablesToCheck) {
            for (const entry of table) {
                ...
                // Match found — encode wParam as Win32 WM_COMMAND from accelerator:
                // HIWORD = 1 (accelerator source), LOWORD = command id
                const cmdWParam = (1 << 16) | (entry.cmdId & 0xFFFF);

                wakaPAC.sendMessage(pacId, MSG_COMMAND, cmdWParam, 0);
    ...
    Object.assign(wakaPAC, {
        ...
        MSG_COMMAND, MSG_COPY, MSG_PASTE, MSG_MOUSEWHEEL, MSG_GESTURE, MSG_SIZE, MSG_MOUSEENTER,

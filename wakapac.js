...
(function () {
    ...
    /**
     * Returns the bounding rectangle of all coordinate-bearing instructions
     * in a display list, or null if the list is empty or contains no geometry.
     * Equivalent to GetEnhMetaFileHeader's rclBounds field in Win32.
     *
     * The returned rect is in display list space (before any playMetaFile offset).
     * Add offsetX/offsetY to convert to container coordinates if needed.
     *
     * @param {Array<Object>} dl - Display list to measure
     * @returns {{ x: number, y: number, width: number, height: number }|null}
     */
    wakaPAC.getMetaFileRect = function(dl) {
        ...
        /**
         * Expands the bounding rect to include a point.
         * @param {number} px
         * @param {number} py
         */
        ...
        for (let i = 0, len = dl.length; i < len; i++) {
            ...
            switch (op.op) {
                ...
                case 'arc':
                case 'ellipse': {
                    // Conservative bound: full circle/ellipse bounding box
                    const rx = op.rx ?? op.r;
                    const ry = op.ry ?? op.r;
                    expand(op.cx - rx, op.cy - ry);
                    expand(op.cx + rx, op.cy + ry);
                    break;
                }
                ...
                case 'clearRect':
                    expand(op.x, op.y);
                    expand(op.x + op.w, op.y + op.h);
                    break;

                case 'bezierCurveTo':
                    expand(op.cp1x, op.cp1y);
                    expand(op.cp2x, op.cp2y);
                    expand(op.x,    op.y);
                    break;

                case 'quadraticCurveTo':
                    expand(op.cpx, op.cpy);
                    expand(op.x,   op.y);
                    break;
